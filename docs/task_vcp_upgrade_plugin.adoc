...
+
Depuis le plug-in Element pour vCenter Server 5:0 :
...
+
Plug-in Element pour vCenter Server 4.10 ou version antérieure :
...
.. (Pour les serveurs internes/sites distants) selon la version du plug-in Element pour vCenter, une URL personnalisée pour le fichier JSON de plug-in ou le plug-in ZIP :
+
...
+
Depuis le plug-in Element pour vCenter Server 5.0, le point d'extension du plug-in distant NetApp Element :
...
+
Pour le plug-in Element pour vCenter Server 4.10 ou version antérieure, NetApp Element Configuration and Management points d'extension :
