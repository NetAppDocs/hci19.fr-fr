...
Si votre noeud de gestion n'est pas connecté à Internet, vous avez téléchargé le https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab["Pack logiciel NetApp HCI pour les clusters de stockage NetApp HCI"^].
...
... Voir  H610S storage nodes to Element 12.3.x or later (phase 2) et terminez le processus pour chaque nœud.
... Réauthentifier si nécessaire et sélectionner *PUT ​/upgrades/{upseId}*.
... Sélectionnez *essayez-le*.
... Saisissez l'ID de mise à niveau de l'étape précédente dans *mise à niveau Id*.
